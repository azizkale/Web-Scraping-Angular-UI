...
export class ProductService {
  ...
  constructor(private pipe: DecimalPipe) {
    this._search$
      .pipe(
        ...
        debounceTime(200),
        switchMap(() => this._search(this.producstarray)),
        delay(200),
  ...
  private _search(productsarray: any[]): Observable<SearchResult> {
    const { sortColumn, sortDirection, pageSize, page, searchTerm } =
      this._state;

    // 1. sort
    let products = sort(this.producstarray, sortColumn, sortDirection);
